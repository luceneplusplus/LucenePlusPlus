...
namespace Lucene
{
	...
	public:
		...
		static void checkNoMatchExplanations(QueryPtr q, const String& defaultFieldName, SearcherPtr searcher, Collection<int32_t> results);
		...
		static void checkHitCollector(QueryPtr query, const String& defaultFieldName, SearcherPtr searcher, Collection<int32_t> results);
		
		/// Tests that a query matches the an expected set of documents using Hits.
		///
		/// Note that when using the Hits API, documents will only be returned if they have a 
		/// positive normalized score.
		static void checkHits(QueryPtr query, const String& defaultFieldName, SearcherPtr searcher, Collection<int32_t> results);
		...
		static void checkHitsQuery(QueryPtr query, Collection<ScoreDocPtr> hits1, Collection<ScoreDocPtr> hits2, Collection<int32_t> results);
		static void checkEqual(QueryPtr query, Collection<ScoreDocPtr> hits1, Collection<ScoreDocPtr> hits2);
		...
		static void checkExplanations(QueryPtr query, const String& defaultFieldName, SearcherPtr searcher, bool deep = false);
