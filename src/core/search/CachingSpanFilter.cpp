...
namespace Lucene
{
    ...
    SpanFilterResultPtr CachingSpanFilter::getCachedResult(IndexReaderPtr reader)
    {
        ...
        SpanFilterResultPtr result(gc_ptr_dynamic_cast<SpanFilterResult>(cache->get(reader, coreKey, delCoreKey)));
        if (result)
        {
            ++hitCount;
            return result;
    ...
    bool CachingSpanFilter::equals(LuceneObjectPtr other)
    {
        ...
        CachingSpanFilterPtr otherCachingSpanFilter(gc_ptr_dynamic_cast<CachingSpanFilter>(other));
