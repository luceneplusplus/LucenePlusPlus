...
namespace Lucene
{
    ...
    void PayloadSpanUtil::queryToSpanQuery(QueryPtr query, Collection<ByteArray> payloads)
    {
        ...
        {
            BooleanQueryPtr booleanQuery(boost::static_pointer_cast<BooleanQuery>(query));
        ...
        {
            TermQueryPtr termQuery(boost::static_pointer_cast<TermQuery>(query));
        ...
        {
            SpanQueryPtr spanQuery(boost::static_pointer_cast<SpanQuery>(query));
            getPayloads(payloads, spanQuery);
        }
        else if (MiscUtils::typeOf<FilteredQuery>(query))
        {
            FilteredQueryPtr filteredQuery(boost::static_pointer_cast<FilteredQuery>(query));
            queryToSpanQuery(filteredQuery->getQuery(), payloads);
        }
        else if (MiscUtils::typeOf<DisjunctionMaxQuery>(query))
        {
            DisjunctionMaxQueryPtr maxQuery(boost::static_pointer_cast<DisjunctionMaxQuery>(query));
        ...
        {
            MultiPhraseQueryPtr multiphraseQuery(boost::static_pointer_cast<MultiPhraseQuery>(query));
                ...
                for (int32_t i = 0; i < disjunctLists.size(); ++i)
                {
                    ...
                    if (disjuncts)
                    {
                        ...
                        for (int32_t j = 0; j < disjuncts.size(); ++j)
                            spanDisjuncts[j] = boost::static_pointer_cast<SpanQuery>(disjuncts[j]);
