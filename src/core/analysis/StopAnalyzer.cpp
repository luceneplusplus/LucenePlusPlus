...
namespace Lucene
{
    ...
    StopAnalyzer::StopAnalyzer(LuceneVersion::Version matchVersion, SetString stopWords)
    {
    ...
    const SetString StopAnalyzer::ENGLISH_STOP_WORDS_SET()
    {
        static SetString __ENGLISH_STOP_WORDS_SET;
        if (!__ENGLISH_STOP_WORDS_SET)
            __ENGLISH_STOP_WORDS_SET = SetString::newStaticInstance(_ENGLISH_STOP_WORDS_SET, _ENGLISH_STOP_WORDS_SET + SIZEOF_ARRAY(_ENGLISH_STOP_WORDS_SET));
    ...
    {
        StopAnalyzerSavedStreamsPtr streams(gc_ptr_dynamic_cast<StopAnalyzerSavedStreams>(getPreviousTokenStream()));
        if (!streams)
        {
