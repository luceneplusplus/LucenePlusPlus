...
namespace Lucene
{
    ...
    int64_t SegmentInfos::getCurrentSegmentGeneration(SetString files)
    {
        if (!files)
            return -1;
        int64_t max = -1;
    ...
    String SegmentInfos::getCurrentSegmentFileName(SetString files)
    {
    ...
    SetString SegmentInfos::files(DirectoryPtr dir, bool includeSegmentsFile)
    {
        SetString files(SetString::newInstance());
