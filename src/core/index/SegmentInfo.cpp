...
namespace Lucene
{
    ...
    int64_t SegmentInfo::sizeInBytes()
    {
        if (_sizeInBytes == -1)
        {
            SetString _files(files());
            _sizeInBytes = 0;
            for (SetString::iterator fileName = _files.begin(); fileName != _files.end(); ++fileName)
            {
    ...
    bool SegmentInfo::hasSeparateNorms()
    {
        if (!normGen)
        {
            ...
            else
            {
                ...
                for (SetString::iterator fileName = result.begin(); fileName != result.end(); ++fileName)
                {
    ...
    void SegmentInfo::addIfExists(SetString files, const String& fileName)
    {
        if (dir->fileExists(fileName))
            files.add(fileName);
    }

    SetString SegmentInfo::files()
    {
        if (_files)
        {
            // already cached
        ...
        {
            for (SetString::iterator ext = IndexFileNames::NON_STORE_INDEX_EXTENSIONS().begin(); ext != IndexFileNames::NON_STORE_INDEX_EXTENSIONS().end(); ++ext)
        ...
        if (docStoreOffset != -1)
        {
            ...
            {
                for (SetString::iterator ext = IndexFileNames::STORE_INDEX_EXTENSIONS().begin(); ext != IndexFileNames::STORE_INDEX_EXTENSIONS().end(); ++ext)
        ...
        else if (preLockless || (!hasSingleNormFile && !useCompoundFile))
        {
            ...
            SetString allFiles(dir->listAll());
            for (SetString::iterator fileName = allFiles.begin(); fileName != allFiles.end(); ++fileName)
            {
