...
namespace Lucene
{
    ...
    void DocInverter::flush(MapDocFieldConsumerPerThreadCollectionDocFieldConsumerPerField threadsAndFields, SegmentWriteStatePtr state)
    {
        ...
        for (MapDocFieldConsumerPerThreadCollectionDocFieldConsumerPerField::iterator entry = threadsAndFields.begin(); entry != threadsAndFields.end(); ++entry)
        {
            ...
            {
                childFields.add(gc_ptr_static_cast<DocInverterPerField>(*perField)->consumer);
                endChildFields.add(gc_ptr_static_cast<DocInverterPerField>(*perField)->endConsumer);
            }

            childThreadsAndFields.put(gc_ptr_static_cast<DocInverterPerThread>(entry->first)->consumer, childFields);
            endChildThreadsAndFields.put(gc_ptr_static_cast<DocInverterPerThread>(entry->first)->endConsumer, endChildFields);
        }
