...
namespace Lucene
{
    MultiLevelSkipListReader::MultiLevelSkipListReader(IndexInputPtr skipStream, int32_t maxSkipLevels, int32_t skipInterval)
    {
        ...
        this->inputIsBuffered = boost::static_pointer_cast<BufferedIndexInput>(skipStream);
    ...
    void MultiLevelSkipListReader::loadSkipLevels()
    {
        ...
        for (int32_t i = numberOfSkipLevels - 1; i > 0; --i)
        {
            ...
            else
            {
                ...
                skipStream[i] = boost::static_pointer_cast<IndexInput>(skipStream[0]->clone());
                if (inputIsBuffered && length < BufferedIndexInput::BUFFER_SIZE)
                    boost::static_pointer_cast<BufferedIndexInput>(skipStream[i])->setBufferSize((int32_t)length);
