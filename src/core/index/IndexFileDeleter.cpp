...
namespace Lucene
{
    ...
    IndexFileDeleter::IndexFileDeleter(DirectoryPtr directory, IndexDeletionPolicyPtr policy, SegmentInfosPtr segmentInfos, InfoStreamPtr infoStream, DocumentsWriterPtr docWriter, SetString synced)
    {
        this->lastFiles = Collection< SetString >::newInstance();
        ...
        SetString files(directory->listAll());
        CommitPointPtr currentCommitPoint;

        for (SetString::iterator fileName = files.begin(); fileName != files.end(); ++fileName)
        {
    ...
    void IndexFileDeleter::deleteCommits()
    {
        if (!commitsToDelete.empty())
        {
            ...
            for (Collection<CommitPointPtr>::iterator commit = commitsToDelete.begin(); commit != commitsToDelete.end(); ++commit)
            {
                ...
                for (SetString::iterator file = (*commit)->files.begin(); file != (*commit)->files.end(); ++file)
                    decRef(*file);
            }
    ...
    void IndexFileDeleter::close()
    {
        ...
        for (Collection< SetString >::iterator file = lastFiles.begin(); file != lastFiles.end(); ++file)
            decRef(*file);
        lastFiles.clear();
        deletePendingFiles();
    }
    ...
    void IndexFileDeleter::deletePendingFiles()
    {
        if (deletable)
        {
            SetString oldDeletable(deletable);
            deletable.reset();
            for (SetString::iterator fileName = oldDeletable.begin(); fileName != oldDeletable.end(); ++fileName)
            {
                if (infoStream)
    ...
    void IndexFileDeleter::incRef(SegmentInfosPtr segmentInfos, bool isCommit)
    {
        ...
        SetString files(segmentInfos->files(directory, isCommit));
        for (SetString::iterator fileName = files.begin(); fileName != files.end(); ++fileName)
            incRef(*fileName);
    }

    void IndexFileDeleter::incRef(SetString files)
    {
    ...
    void IndexFileDeleter::decRef(SetString files)
    {
        for (SetString::iterator file = files.begin(); file != files.end(); ++file)
            decRef(*file);
    }
    ...
    void IndexFileDeleter::deleteFiles(SetString files)
    {
        for (SetString::iterator file = files.begin(); file != files.end(); ++file)
            deleteFile(*file);
    }

    void IndexFileDeleter::deleteNewFiles(SetString files)
    {
        for (SetString::iterator fileName = files.begin(); fileName != files.end(); ++fileName)
        {
    ...
    void IndexFileDeleter::deleteFile(const String& fileName)
    {
        ...
        catch (IOException& e) // if delete fails
        {
            if (directory->fileExists(fileName)) // if delete fails
            {
                ...
                if (!deletable)
                    deletable = SetString::newInstance();
    ...
    CommitPoint::CommitPoint(Collection<CommitPointPtr> commitsToDelete, DirectoryPtr directory, SegmentInfosPtr segmentInfos)
    {
        ...
        this->files = SetString::newInstance(files.begin(), files.end());
    ...
    SetString CommitPoint::getFileNames()
    {
        return files;
    }
