...
namespace Lucene
{
    ...
    void DirectoryReader::doCommit(MapStringString commitUserData)
    {
        if (_hasChanges)
        {
            ...
            try
            {
                ...
                SetString files(segmentInfos->files(_directory, false));
    ...
    SetString DirectoryReader::getFieldNames(FieldOption fieldOption)
    {
        ensureOpen();
        return getFieldNames(fieldOption, Collection<IndexReaderPtr>::newInstance(subReaders.begin(), subReaders.end()));
    }

    SetString DirectoryReader::getFieldNames(FieldOption fieldOption, Collection<IndexReaderPtr> subReaders)
    {
        // maintain a unique set of field names
        SetString fieldSet(SetString::newInstance());
    ...
    ReaderCommit::ReaderCommit(SegmentInfosPtr infos, DirectoryPtr dir)
    {
        ...
        SetString files(infos->files(dir, true));
    ...
    SetString ReaderCommit::getFileNames()
    {
        return files;
    }
