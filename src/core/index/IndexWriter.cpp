...
namespace Lucene
{
    ...
    void IndexWriter::initialize()
    {
        ...
        synced = SetString::newInstance();
        syncing = SetString::newInstance();
        changeCount = 0;
    ...
    bool IndexWriter::flushDocStores()
    {
        ...
        if (useCompoundDocStore && !docStoreSegment.empty() && !closedFiles.empty())
        {
            ...
            try
            {
                ...
                for (SetString::iterator file = closedFiles.begin(); file != closedFiles.end(); ++file)
        ...
        try
        {
            ...
            if (!success)
            {
                    ...
                    message(L"hit exception adding document");
                {
                    ...
                    if (docWriter)
                    {
                        SetString files(docWriter->abortedFiles());
                        if (files)
    ...
    bool IndexWriter::startSync(const String& fileName, SetString pending)
    {
    ...
    bool IndexWriter::waitForAllSynced(SetString syncing)
    {
        SyncLock syncedLock(&synced);
        for (SetString::iterator fileName = syncing.begin(); fileName != syncing.end(); ++fileName)
        {
        ...
        try
        {
...

            {
                ...
                try
                {
                    ...
                    SetString files(toSync->files(directory, false));
            ...
            try
            {
                ...
                while (true)
                {
                    SetString pending(SetString::newInstance());
                    SetString files(toSync->files(directory, false));
                    for (SetString::iterator fileName = files.begin(); fileName != files.end(); ++fileName)
                    {
    ...
    {
        this->readerMap = MapSegmentInfoSegmentReader::newInstance();
    ...
    {
        return get(info, doOpenStores, BufferedIndexInput::BUFFER_SIZE, indexWriter->readerTermsIndexDivisor);
    }
