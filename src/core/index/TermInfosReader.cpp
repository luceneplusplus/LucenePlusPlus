...
namespace Lucene
{
    ...
    TermInfosReaderThreadResourcesPtr TermInfosReader::getThreadResources()
    {
        ...
        if (!resources)
        {
            ...
            resources->termInfoCache = new_gc<TermInfoCache>(DEFAULT_CACHE_SIZE);
    ...
    {
        return gc_ptr_static_cast<SegmentTermEnum>(origEnum->clone());
    }
    ...
    SegmentTermEnumPtr TermInfosReader::terms(TermPtr term)
    {
        ...
        get(term, false);
        return gc_ptr_static_cast<SegmentTermEnum>(getThreadResources()->termEnum->clone());
    }
