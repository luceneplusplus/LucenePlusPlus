...
namespace Lucene
{
    ...
    void StoredFieldsWriter::closeDocStore(SegmentWriteStatePtr state)
    {
        ...
        if (fieldsWriter)
        {
            ...
            state->docWriter->removeOpenFile(state->docStoreSegmentName + L"." + IndexFileNames::FIELDS_EXTENSION());
            state->docWriter->removeOpenFile(state->docStoreSegmentName + L"." + IndexFileNames::FIELDS_INDEX_EXTENSION());
    ...
    {
        SyncLock syncLock(this);
        BOOST_ASSERT(docWriter->writer->testPoint(L"StoredFieldsWriter.finishDocument start"));
        initFieldsWriter();
    ...
    StoredFieldsWriterPerDoc::StoredFieldsWriterPerDoc(StoredFieldsWriterPtr fieldsWriter)
    {
        ...
        buffer = fieldsWriter->docWriter->newPerDocBuffer();
    ...
    {
        fieldsWriter->finishDocument(LuceneThis());
    }
}
