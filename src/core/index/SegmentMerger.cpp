...
namespace Lucene
{
    ...
    SetString SegmentMerger::getMergedFiles()
    {
        SetString fileSet(SetString::newInstance());

        // Basic files
        for (SetString::iterator ext = IndexFileNames::COMPOUND_EXTENSIONS().begin(); ext != IndexFileNames::COMPOUND_EXTENSIONS().end(); ++ext)
        {
        ...
        {
            for (SetString::iterator ext = IndexFileNames::VECTOR_EXTENSIONS().begin(); ext != IndexFileNames::VECTOR_EXTENSIONS().end(); ++ext)
                fileSet.add(segment + L"." + *ext);
        }
    ...
    SetString SegmentMerger::createCompoundFile(const String& fileName)
    {
        SetString files(getMergedFiles());
    ...
    void SegmentMerger::addIndexed(IndexReaderPtr reader, FieldInfosPtr fInfos, SetString names,
    ...
    {
        for (SetString::iterator field = names.begin(); field != names.end(); ++field)
        {
