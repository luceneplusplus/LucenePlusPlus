...
static void performDefaultTests(IndexReaderPtr r1)
{
    ...
    IndexReaderPtr pr1Clone = gc_ptr_dynamic_cast<IndexReader>(r1->clone());
...
BOOST_AUTO_TEST_CASE(testCloneReadOnlySegmentReader)
{
    ...
    IndexReaderPtr readOnlyReader = gc_ptr_dynamic_cast<IndexReader>(reader->clone(true));
...
BOOST_AUTO_TEST_CASE(testCloneNoChangesStillReadOnly)
{
    ...
    IndexReaderPtr r2 = gc_ptr_dynamic_cast<IndexReader>(r1->clone(false));
...
BOOST_AUTO_TEST_CASE(testCloneReadOnlyToWriteable)
{
    ...
    IndexReaderPtr reader2 = gc_ptr_dynamic_cast<IndexReader>(reader1->clone(false));
...
BOOST_AUTO_TEST_CASE(testReadOnlyCloneAfterOptimize)
{
    ...
    w->optimize();
    w->close();
    IndexReaderPtr reader2 = gc_ptr_dynamic_cast<IndexReader>(reader1->clone(true));
    BOOST_CHECK(isReadOnly(reader2));
    reader1->close();
    reader2->close();
    dir1->close();
...
BOOST_AUTO_TEST_CASE(testSegmentReaderCloseReferencing)
{
    ...
    SegmentReaderPtr clonedSegmentReader = gc_ptr_dynamic_cast<SegmentReader>(origSegmentReader->clone());
...
BOOST_AUTO_TEST_CASE(testSegmentReaderDelDocsReferenceCounting)
{
    ...
    IndexReaderPtr clonedReader = gc_ptr_dynamic_cast<IndexReader>(origReader->clone());
    ...
    IndexReaderPtr cloneReader2 = gc_ptr_dynamic_cast<IndexReader>(reopenedReader->clone());
...
BOOST_AUTO_TEST_CASE(testCloneWithSetNorm)
{
    ...
    // the cloned segmentreader should have 2 references, 1 to itself, and 1 to the original segmentreader
    IndexReaderPtr clonedReader = gc_ptr_dynamic_cast<IndexReader>(orig->clone());
    orig->close();
    clonedReader->close();
...
BOOST_AUTO_TEST_CASE(testCloneSubreaders)
{
    ...
    for (int32_t x = 0; x < subs.size(); ++x)
        clones[x] = gc_ptr_dynamic_cast<IndexReader>(subs[x]->clone());
    reader->close();
    for (int32_t x = 0; x < subs.size(); ++x)
        clones[x]->close();
    dir1->close();
