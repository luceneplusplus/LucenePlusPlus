...
BOOST_AUTO_TEST_CASE(testCommitUserData)
{
    ...
    commitUserData.put(String(L"foo"), String(L"fighters"));
...
BOOST_AUTO_TEST_CASE(testGetFieldNames)
{
    ...
    IndexReaderPtr reader = IndexReader::open(d, false);
    BOOST_CHECK(reader->isCurrent());
    SetString fieldNames = reader->getFieldNames(IndexReader::FIELD_OPTION_ALL);
...
BOOST_AUTO_TEST_CASE(testBinaryFields)
{
    ...
    SetString lazyFields = SetString::newInstance();
    lazyFields.add(L"bin1");
    FieldSelectorPtr sel = newLucene<SetBasedFieldSelector>(SetString::newInstance(), lazyFields);
...
BOOST_AUTO_TEST_CASE(testNoDupCommitFileNames)
{
    ...
    {
        SetString files = (*commit)->getFileNames();
        SetString seen = SetString::newInstance();
...
BOOST_AUTO_TEST_CASE(testFieldCacheReuseAfterClone)
{
    ...
    // Clone reader
    IndexReaderPtr r2 = gc_ptr_dynamic_cast<IndexReader>(r->clone());
    r->close();
    BOOST_CHECK_NE(r2, r);
...
BOOST_AUTO_TEST_CASE(testReopenChangeReadonly)
{
    ...
    // Open reader1
    IndexReaderPtr r = IndexReader::open(dir, false);
    BOOST_CHECK(gc_ptr_dynamic_cast<DirectoryReader>(r));
    IndexReaderPtr r1 = SegmentReader::getOnlySegmentReader(r);
    Collection<int32_t> ints = FieldCache::DEFAULT()->getInts(r1, L"number");
    BOOST_CHECK_EQUAL(1, ints.size());
    BOOST_CHECK_EQUAL(17, ints[0]);
    ...
    BOOST_CHECK(gc_ptr_dynamic_cast<ReadOnlyDirectoryReader>(r3));
    r3->close();
    ...
    r->close();
    BOOST_CHECK(gc_ptr_dynamic_cast<ReadOnlyDirectoryReader>(r2));
    ...
    BOOST_CHECK(gc_ptr_dynamic_cast<ReadOnlySegmentReader>(subs[0]));
    BOOST_CHECK(gc_ptr_dynamic_cast<ReadOnlySegmentReader>(subs[1]));
    BOOST_CHECK(ints.equals(ints2));
...
BOOST_AUTO_TEST_CASE(testNoTermsIndex)
{
    ...
    BOOST_CHECK(!gc_ptr_dynamic_cast<SegmentReader>(r->getSequentialSubReaders()[0])->termsIndexLoaded());
    BOOST_CHECK_EQUAL(-1, gc_ptr_dynamic_cast<SegmentReader>(r->getSequentialSubReaders()[0])->getTermInfosIndexDivisor());
    ...
    {
        SegmentReaderPtr subReader = gc_ptr_dynamic_cast<SegmentReader>(*sub);
