...
{
    MockRAMDirectoryPtr dir = boost::static_pointer_cast<MockRAMDirectory>(writer->getDirectory());
    ConcurrentMergeSchedulerPtr cms = boost::static_pointer_cast<ConcurrentMergeScheduler>(writer->getMergeScheduler());
    dir->crash();
    cms->sync();
    dir->clearCrash();
}
