...
{
    MockRAMDirectoryPtr dir = LuceneDynamicCast<MockRAMDirectory>(writer->getDirectory());
    ConcurrentMergeSchedulerPtr cms = LuceneDynamicCast<ConcurrentMergeScheduler>(writer->getMergeScheduler());
    dir->crash();
    cms->sync();
    dir->clearCrash();
}
