...
{
    MockRAMDirectoryPtr dir = gc_ptr_dynamic_cast<MockRAMDirectory>(writer->getDirectory());
    ConcurrentMergeSchedulerPtr cms = gc_ptr_dynamic_cast<ConcurrentMergeScheduler>(writer->getMergeScheduler());
    dir->crash();
    cms->sync();
    dir->clearCrash();
}
