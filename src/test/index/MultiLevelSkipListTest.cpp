...
public:
    ...
    {
        return newLucene<CountingStream>(boost::static_pointer_cast<IndexInput>(input->clone()));
    }
};
...
BOOST_AUTO_TEST_CASE(testSimpleSkip)
{
    ...
    SegmentTermPositionsPtr tp = boost::static_pointer_cast<SegmentTermPositions>(reader->termPositions());
