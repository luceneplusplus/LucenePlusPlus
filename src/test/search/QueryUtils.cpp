...
namespace Lucene
{
    ...
    {
        QueryPtr q2 = gc_ptr_dynamic_cast<Query>(q->clone());
        checkEqual(q, q2);

        QueryPtr q3 = gc_ptr_dynamic_cast<Query>(q->clone());
    ...
    void QueryUtils::check(QueryPtr q1, SearcherPtr s, bool wrap)
    {
        ...
        if (s)
        {
            IndexSearcherPtr is = gc_ptr_dynamic_cast<IndexSearcher>(s);
            if (is)
            {
            ...
            QueryPtr q2 = gc_ptr_dynamic_cast<Query>(q1->clone());
    ...
    RAMDirectoryPtr QueryUtils::makeEmptyIndex(int32_t numDeletedDocs)
    {
        ...
        if (0 < numDeletedDocs)
            BOOST_CHECK(w->hasDeletions());

        BOOST_CHECK_EQUAL(numDeletedDocs, w->maxDoc());
        BOOST_CHECK_EQUAL(0, w->numDocs());
        w->close();
        IndexReaderPtr r = IndexReader::open(d, true);
        BOOST_CHECK_EQUAL(numDeletedDocs, r->numDeletedDocs());
        r->close();
        return d;
    }
        ...
        public:
            ...
            virtual void collect(int32_t doc)
            {
                ...
                for (int32_t i = lastDoc[0] + 1; i <= doc; ++i)
                {
                    ...
                    BOOST_CHECK(scorer->advance(i) != DocIdSetIterator::NO_MORE_DOCS);
                    BOOST_CHECK_EQUAL(doc, scorer->docID());
                    double skipToScore = scorer->score();
                    BOOST_CHECK_CLOSE_FRACTION(skipToScore, scorer->score(), 1e-5);
                    BOOST_CHECK_CLOSE_FRACTION(score, skipToScore, 1e-5);
                }
                lastDoc[0] = doc;
            }
