...
BOOST_AUTO_TEST_CASE(testWildcard)
{
    ...
    FuzzyQueryPtr fq = boost::static_pointer_cast<FuzzyQuery>(getQuery(L"term~0.7", AnalyzerPtr()));
    ...
    fq = boost::static_pointer_cast<FuzzyQuery>(getQuery(L"term~", AnalyzerPtr()));
...
BOOST_AUTO_TEST_CASE(testRange)
{
    ...
    BOOST_CHECK_EQUAL(MultiTermQuery::CONSTANT_SCORE_AUTO_REWRITE_DEFAULT(), boost::static_pointer_cast<TermRangeQuery>(getQuery(L"[ a TO z]", AnalyzerPtr()))->getRewriteMethod());

    QueryParserPtr qp = newLucene<QueryParser>(LuceneVersion::LUCENE_CURRENT, L"field", newLucene<SimpleAnalyzer>());
    qp->setMultiTermRewriteMethod(MultiTermQuery::SCORING_BOOLEAN_QUERY_REWRITE());
    BOOST_CHECK_EQUAL(MultiTermQuery::SCORING_BOOLEAN_QUERY_REWRITE(), boost::static_pointer_cast<TermRangeQuery>(qp->parse(L"[ a TO z]"))->getRewriteMethod());
...
BOOST_AUTO_TEST_CASE(testStarParsing)
{
    ...
    TermQueryPtr tq = boost::static_pointer_cast<TermQuery>(qp->parse(L"foo:zoo*"));
    BOOST_CHECK_EQUAL(L"zoo", tq->getTerm()->text());
    BOOST_CHECK_EQUAL(2, qp->type[0]);

    tq = boost::static_pointer_cast<TermQuery>(qp->parse(L"foo:zoo*^2"));
    BOOST_CHECK_EQUAL(L"zoo", tq->getTerm()->text());
    BOOST_CHECK_EQUAL(2, qp->type[0]);
    BOOST_CHECK_EQUAL(tq->getBoost(), 2);

    tq = boost::static_pointer_cast<TermQuery>(qp->parse(L"foo:*"));
    BOOST_CHECK_EQUAL(L"*", tq->getTerm()->text());
    BOOST_CHECK_EQUAL(1, qp->type[0]);  // could be a valid prefix query in the future too

    tq = boost::static_pointer_cast<TermQuery>(qp->parse(L"foo:*^2"));
    BOOST_CHECK_EQUAL(L"*", tq->getTerm()->text());
    ...
    tq = boost::static_pointer_cast<TermQuery>(qp->parse(L"*:*"));
    BOOST_CHECK_EQUAL(L"*", tq->getTerm()->field());
    BOOST_CHECK_EQUAL(L"*", tq->getTerm()->text());
...
BOOST_AUTO_TEST_CASE(testStopwords)
{
    ...
    BOOST_CHECK(boost::static_pointer_cast<BooleanQuery>(result)->getClauses().empty());
    ...
    BOOST_CHECK(result);
    BOOST_CHECK(MiscUtils::typeOf<BooleanQuery>(result));
    BOOST_CHECK_EQUAL(boost::static_pointer_cast<BooleanQuery>(result)->getClauses().size(), 2);
}
...
BOOST_AUTO_TEST_CASE(testPositionIncrement)
{
    ...
    PhraseQueryPtr pq = boost::static_pointer_cast<PhraseQuery>(qp->parse(qtxt));
...
BOOST_AUTO_TEST_CASE(testMatchAllDocs)
{
    ...
    BOOST_CHECK(newLucene<MatchAllDocsQuery>()->equals(qp->parse(L"(*:*)")));
    BooleanQueryPtr bq = boost::static_pointer_cast<BooleanQuery>(qp->parse(L"+*:* -*:*"));
