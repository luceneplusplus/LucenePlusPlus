...
namespace Lucene
{
    ...
    ByteArray MemoryIndexReader::norms(const String& field)
    {
        ...
        if (field != cachedFieldName || sim != cachedSimilarity) // not cached?
        {
            ...
            uint8_t norm = sim->encodeNormValue(n);
    ...
    bool MemoryIndexTermEnum::next()
    {
        ...
        if (srtFldsIdx >= reader->memoryIndex->sortedFields.size())
            return false;
        MemoryIndexInfoPtr info(reader->getInfo(srtFldsIdx));
        if (++srtTermsIdx < info->sortedTerms.size())
            return true;
        ...
        if (srtFldsIdx >= reader->memoryIndex->sortedFields.size())
            return false;
        reader->getInfo(srtFldsIdx)->sortTerms();
        return true;
    }
    ...
    TermPtr MemoryIndexTermEnum::term()
    {
        ...
        if (srtTermsIdx >= info->sortedTerms.size())
            return TermPtr();
        return createTerm(info, srtFldsIdx, info->sortedTerms[srtTermsIdx].first);
    }
    ...
    int32_t MemoryIndexTermEnum::docFreq()
    {
        ...
        if (srtTermsIdx >= info->sortedTerms.size())
            return 0;
        return reader->memoryIndex->numPositions(info->getPositions(srtTermsIdx));
    }
