...
namespace Lucene
{
    ...
    TermPtr MemoryIndexReader::MATCH_ALL_TERM()
    {
        ...
        if (!_MATCH_ALL_TERM)
            _MATCH_ALL_TERM = newStaticLucene<Term>(L"");
    ...
    SetString MemoryIndexReader::getFieldNames(FieldOption fieldOption)
    {
        static SetString emptySet;
        if (!emptySet)
            emptySet = SetString::newStaticInstance();
