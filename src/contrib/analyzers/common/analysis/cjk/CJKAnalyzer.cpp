...
namespace Lucene
{
    ...
    CJKAnalyzer::CJKAnalyzer(LuceneVersion::Version matchVersion, SetString stopwords)
    {
    ...
    const SetString CJKAnalyzer::getDefaultStopSet()
    {
        static SetString stopSet;
        if (!stopSet)
            stopSet = SetString::newStaticInstance(_STOP_WORDS, _STOP_WORDS + SIZEOF_ARRAY(_STOP_WORDS));
        return stopSet;
    }
    ...
    {
        CJKAnalyzerSavedStreamsPtr streams(gc_ptr_dynamic_cast<CJKAnalyzerSavedStreams>(getPreviousTokenStream()));
        if (!streams)
        {
