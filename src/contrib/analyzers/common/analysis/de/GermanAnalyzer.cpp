...
namespace Lucene
{
    ...
    GermanAnalyzer::GermanAnalyzer(LuceneVersion::Version matchVersion, SetString stopwords)
    {
        this->stopSet = stopwords;
        this->matchVersion = matchVersion;
    }

    GermanAnalyzer::GermanAnalyzer(LuceneVersion::Version matchVersion, SetString stopwords, SetString exclusions)
    {
        this->stopSet = stopwords;
    ...
    const SetString GermanAnalyzer::getDefaultStopSet()
    {
        static SetString stopSet;
        if (!stopSet)
            stopSet = SetString::newStaticInstance(_GERMAN_STOP_WORDS, _GERMAN_STOP_WORDS + SIZEOF_ARRAY(_GERMAN_STOP_WORDS));
        return stopSet;
    }

    void GermanAnalyzer::setStemExclusionTable(SetString exclusions)
    {
    ...
    {
        GermanAnalyzerSavedStreamsPtr streams(gc_ptr_dynamic_cast<GermanAnalyzerSavedStreams>(getPreviousTokenStream()));
        if (!streams)
        {
