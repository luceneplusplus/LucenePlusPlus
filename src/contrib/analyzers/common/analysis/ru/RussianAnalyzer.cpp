...
namespace Lucene
{
    ...
    RussianAnalyzer::RussianAnalyzer(LuceneVersion::Version matchVersion, SetString stopwords)
    {
    ...
    const SetString RussianAnalyzer::getDefaultStopSet()
    {
        ...
        if (!stopSet)
        {
            ...
            stopSet = SetString::newStaticInstance(words.begin(), words.end());
        }
        return stopSet;
    }
    ...
    {
        RussianAnalyzerSavedStreamsPtr streams(gc_ptr_dynamic_cast<RussianAnalyzerSavedStreams>(getPreviousTokenStream()));
        if (!streams)
        {
