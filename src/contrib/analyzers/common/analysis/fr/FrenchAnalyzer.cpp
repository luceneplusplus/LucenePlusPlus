...
namespace Lucene
{
    ...
    FrenchAnalyzer::FrenchAnalyzer(LuceneVersion::Version matchVersion, SetString stopwords)
    {
        this->stoptable = stopwords;
        this->matchVersion = matchVersion;
    }

    FrenchAnalyzer::FrenchAnalyzer(LuceneVersion::Version matchVersion, SetString stopwords, SetString exclusions)
    {
        this->stoptable = stopwords;
    ...
    const SetString FrenchAnalyzer::getDefaultStopSet()
    {
        static SetString stoptable;
        if (!stoptable)
            stoptable = SetString::newStaticInstance(_FRENCH_STOP_WORDS, _FRENCH_STOP_WORDS + SIZEOF_ARRAY(_FRENCH_STOP_WORDS));
        return stoptable;
    }

    void FrenchAnalyzer::setStemExclusionTable(SetString exclusions)
    {
