...
namespace Lucene
{
    ...
    GreekAnalyzer::GreekAnalyzer(LuceneVersion::Version matchVersion, SetString stopwords)
    {
    ...
    const SetString GreekAnalyzer::getDefaultStopSet()
    {
    ...
    {
        GreekAnalyzerSavedStreamsPtr streams(gc_ptr_dynamic_cast<GreekAnalyzerSavedStreams>(getPreviousTokenStream()));
        if (!streams)
        {
