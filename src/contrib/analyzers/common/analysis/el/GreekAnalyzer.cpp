...
namespace Lucene
{
    ...
    GreekAnalyzer::GreekAnalyzer(LuceneVersion::Version matchVersion) : StopwordAnalyzerBase(matchVersion, getDefaultStopSet())
    {
    }

    GreekAnalyzer::GreekAnalyzer(LuceneVersion::Version matchVersion, HashSet<String> stopwords) : StopwordAnalyzerBase(matchVersion, stopwords)
    {
    }
    ...
    TokenStreamComponentsPtr GreekAnalyzer::createComponents(const String& fieldName, ReaderPtr reader)
    {
        TokenizerPtr source(newLucene<StandardTokenizer>(matchVersion, reader));
        TokenStreamPtr result(newLucene<GreekLowerCaseFilter>(matchVersion, source));
        ...
        if (LuceneVersion::onOrAfter(matchVersion, LuceneVersion::LUCENE_31))
            result = newLucene<GreekStemFilter>(result);
