...
namespace Lucene
{
    ...
    BrazilianAnalyzer::BrazilianAnalyzer(LuceneVersion::Version matchVersion, SetString stopwords)
    {
        this->stoptable = stopwords;
        this->matchVersion = matchVersion;
    }

    BrazilianAnalyzer::BrazilianAnalyzer(LuceneVersion::Version matchVersion, SetString stopwords, SetString exclusions)
    {
        this->stoptable = stopwords;
    ...
    const SetString BrazilianAnalyzer::getDefaultStopSet()
    {
        static SetString stopSet;
        if (!stopSet)
            stopSet = SetString::newStaticInstance(_BRAZILIAN_STOP_WORDS, _BRAZILIAN_STOP_WORDS + SIZEOF_ARRAY(_BRAZILIAN_STOP_WORDS));
        return stopSet;
    }

    void BrazilianAnalyzer::setStemExclusionTable(SetString exclusions)
    {
