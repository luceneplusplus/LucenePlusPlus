...
namespace Lucene
{
    ...
    public:
        /// A convenience method that tries to first get a TermPositionVector for
        /// the specified docId, then, falls back to using the passed in {@link
        /// Document} to retrieve the TokenStream.  This is useful when you already
        /// have the document, but would prefer to use the vector first.
        /// @param reader The {@link IndexReader} to use to try and get the vector
        /// from.
        /// @param docId The docId to retrieve.
        /// @param field The field to retrieve on the document.
        /// @param doc The document to fall back on.
        /// @param analyzer The analyzer to use for creating the TokenStream if
        /// the vector doesn't exist.
        /// @return The {@link TokenStream} for the {@link Fieldable} on the
        /// {@link Document}
        ...
        /// A convenience method that tries a number of approaches to getting a
        /// token stream.  The cost of finding there are no termVectors in the
        /// index is minimal (1000 invocations still registers 0 ms). So this
        /// "lazy" (flexible?) approach to coding is probably acceptable
        ...
        ///
        /// In my tests the speeds to recreate 1000 token streams using this
        /// method are:
        /// - with TermVector offset only data stored - 420  milliseconds
        /// - with TermVector offset AND position data stored - 271 milliseconds
        /// (nb timings for TermVector with position data are based on a tokenizer
        /// with contiguous positions - no overlaps or gaps)  The cost of not using
        /// TermPositionVector to store pre-parsed content and using an analyzer
        /// to re-parse the original content:
        ...
        /// 3) Use of compression on field storage - could be faster due to
        ...
        /// numbers have no overlaps or gaps. If looking to eek out the last drops
        /// of performance, set to true.  If in doubt, set to false.
