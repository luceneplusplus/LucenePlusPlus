...
namespace Lucene
{
    ...
    /// Overview
    ///
    /// This class is a replacement/substitute for a large subset of {@link
    /// RAMDirectory} functionality. It is designed to enable maximum efficiency
    /// for on-the-fly matchmaking combining structured and fuzzy fulltext search
    /// in realtime streaming applications such as Nux XQuery based XML message
    /// queues, publish-subscribe systems for Blogs/newsfeeds, text chat, data
    /// acquisition and distribution systems, application level routers, firewalls,
    /// classifiers, etc.  Rather than targeting fulltext search of infrequent
    /// queries over huge persistent data archives (historic search), this class
    /// targets fulltext search of huge numbers of queries over comparatively
    /// small transient realtime data (prospective search).
    ///
    /// For example as in
    /// <pre>
    ...
    /// value.  The fulltext value is tokenized (split and transformed) into zero
    /// or more index terms (aka words) on addField(), according to the policy
    /// implemented by an Analyzer.  For example, Lucene analyzers can split on
    /// whitespace, normalize to lower case for case insensitivity, ignore common
    /// terms with little discriminatory value such as "he", "in", "and" (stop
    /// words), reduce the terms to their natural linguistic root form such as
    /// "fishing" being reduced to "fish" (stemming), resolve synonyms / inflexions
    /// / thesauri (upon indexing and/or querying), etc.
    ///
    /// Note that a Lucene query selects on the field names and associated
    /// (indexed) tokenized terms, not on the original fulltext(s) - the latter
    /// are not stored but rather thrown away immediately after tokenization.
    ///
    /// For some interesting background information on search technology, see
    /// Bob Wyman's <a target="_blank" href="http://bobwyman.pubsub.com/main/2005/
    /// 05/mary_hodder_poi.html">Prospective Search</a>,
    /// Jim Gray's <a target="_blank" href="http://www.acmqueue.org/modules.php
    /// ?name=Content&pa=showpage&pid=293&page=4"> A Call to Arms - Custom
    /// subscriptions</a>, and Tim Bray's <a target="_blank" href="http://www.
    /// tbray.org/ongoing/When/200x/2003/07/30/OnSearchTOC">On Search, the
    /// Series</a>.
    ///
    ///
    /// Example Usage
    ...
    /// index->addField(L"content", L"Readings about Salmons and other"
    ///                             L" select Alaska fishing Manuals", analyzer);
    /// index->addField(L"author", L"Tales of James", analyzer);
    /// QueryParserPtr parser = newLucene<QueryParser>(L"content", analyzer);
    /// double score = index->search(parser->parse(L"+author:james +salmon~"
    ///                                            L" +fish* manual~"));
    /// if (score > 0.0)
    /// {
    ///     // it's a match
    ...
    /// Lucene framework.
    ///
    /// This class performs very well for very small texts (eg. 10 chars) as well
    /// as for large texts (eg. 10 MB) and everything in between.  Typically, it
    /// is about 10-100 times faster than RAMDirectory.  Note that RAMDirectory
    /// has particularly large efficiency overheads for small to medium sized
    /// texts, both in time and space.  Indexing a field with N tokens takes O(N)
    /// in the best case, and O(N logN) in the worst case.  Memory consumption is
    /// probably larger than for RAMDirectory.
    ///
    class LPPCONTRIBAPI MemoryIndex : public LuceneObject
    {
    public:
        /// Constructs an empty instance that can optionally store the start and
        /// end character offset of each token term in the text.  This can be
        /// useful for highlighting of hit locations with the Lucene highlighter
        /// package.  Private until the highlighter package matures, so that
        /// this can actually be meaningfully integrated.
        /// @param storeOffsets Whether or not to store the start and end
    ...
    public:
        /// Convenience method; Tokenizes the given field text and adds the
        /// resulting terms to the index; Equivalent to adding an indexed
        /// non-keyword Lucene {@link Field} that is {@link Field::INDEX_ANALYZED
        /// tokenized}, {@link Field::STORE_NO not stored}, {@link
        /// Field::TERM_VECTOR_WITH_POSITIONS termVectorStored with positions}
        /// (or {@link Field::TERM_VECTOR_WITH_POSITIONS_OFFSETS termVectorStored
        ...
        /// Iterates over the given token stream and adds the resulting terms to
        /// the index; Equivalent to adding a tokenized, indexed, termVectorStored,
        /// unstored, Lucene {@link Field}.  Finally closes the token stream.
        /// Note that untokenized keywords can be added with this method via
        ...
        /// [0.0 .. 1.0], with 0.0 indicating no match. The higher the number the
        /// better the match.
    ...
    public:
        ...
        /// Note that sorting is lazily delayed until required (often it's not
        /// required at all).
        void sortTerms();
